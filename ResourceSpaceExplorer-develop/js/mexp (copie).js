...
media.view.MEXP = media.View.extend({
	...
	render: function() {
		...
		selection.each( function( model ) {
			var id = '#rse-item-' + this.service.id + '-' + this.tab + '-' + model.get( 'id' );
	...
	fetchedEmpty: function( response ) {
		...
		this.$el.find( '.rse-empty' ).text( mexp.labels.noresults ).show();
		this.$el.find( '.rse-pagination' ).hide();
...
media.view.MediaFrame.Post = post_frame.extend({
	...
	mexpContentRender : function( service, tab ) {
		...
		this.content.set( new media.view.MEXP( {
			...
			tab        : tab,
			className  : 'clearfix attachments-browser rse-content rse-content-' + service.id + ' rse-content-' + service.id + '-' + tab
		} ) );
